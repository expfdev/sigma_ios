...
- `retryCount: UInt` -  количество повторных попыток запрашивания конфигурации при неудачном запросе. Необязательный параметр. По умолчанию равен 0.
- `fetchTimeout: TimeInterval` - таймаут для внутренних запросов SDK к Sigma API. По умолчанию равен `URLSessionConfiguration.default.timeoutIntervalForRequest` (60 секунд). При попытке установить таймаут меньше, чем 0.5 секунд, используется значение по умолчанию (60 секунд)
...
### 1.8.1
- Значение по умолчанию параметра `fetchTimeout` в `Sigma.initializeClient` исправлено с 5 секунд на `URLSessionConfiguration.default.timeoutIntervalForRequest` (60 секунд)

### 1.8.0
- Добавлен параметр `fetchTimeout` в `Sigma.initializeClient` - таймаут для внутренних запросов SDK к Sigma API. По умолчанию равен 5 секундам. При попытке установить таймаут меньше, чем 0.5 секунд, используется значение по умолчанию (5 секунд).

### 1.7.1
- Исправлена ошибка обработки платформы для Feature Flag.

### 1.7.0
- Значение по умолчанию параметра `retryCount` в `Sigma.initializeClient` изменено с 3 на 0.
- Добавлена обработка флага `use_geo` из файла конфигурации. Если флаг присутствует и равен `false`, запрос геолокации осуществлен не будет.

### 1.6.0
- Добавлены методы `SigmaClient.getUserExperimentsDetails`, возвращающие массив экспериментов, в которые попал пользователь.
